module BP3D.Core {
  ...
  export class Utils {
     ...
     */
    public static getPointDistanceFromLine(x: number, y: number, x1: number, y1: number, x2: number, y2: number): number {
      var tPoint = Utils.getClosestPointOnLine(x, y, x1, y1, x2, y2);
      var tDx = x - tPoint.x;
      var tDy = y - tPoint.y;
     ...
     * @returns The point.
     */
    static getClosestPointOnLine(x: number, y: number, x1: number, y1: number, x2: number, y2: number): { x: number, y: number } {
     ...
     */
    static isClockwise(points: Array<{x: number, y: number}>): boolean {
      // make positive
    ...
    */
    ...
    static map<T, U>(array: T[], func: (element: T) => U): U[] {
      var tResult: U[] = [];
    ...
    static unique<T>(arr: T[], hashFunc: (element: T) => string): T[] {
      ...
      for (var tI = 0; tI < arr.length; tI++) {
        if (!tMap.hasOwnProperty(hashFunc(arr[tI]))) {
